"""
Modified RFEM on the CLIP VISION encoder (ViT-B/32) — sink-aware K-sigma filter.

Identical to rfem_clip_vision_pipeline.py EXCEPT in Step 2 (K-sigma filter):

  Standard version: μ_h, σ_h computed over the full 50×50 rolled matrix.
  Problem: The (0,0) entry — CLS attending to itself — accumulates a very
           large rolled value (~12 after 12 layers of identity accumulation),
           inflating σ_h and pushing the threshold above the patch-token
           scale → all patch tokens get killed at K≥0.3.

  Sink-aware fix (Prof. Benois-Pineau, May 2026):
    μ_h, σ_h are computed from the CLS ROW ONLY (row 0),
    excluding column 0 (the CLS-self entry).
    i.e. calibrate the threshold against the 49 patch scores in exactly
    the row we read out for spatial importance.
    The threshold is then applied to the FULL T×T matrix.

Everything else — weighted aggregation, value-preserving filter,
one PDF per figure, per-image subfolders, Reds colormap — is
identical to rfem_clip_vision_pipeline.py.
"""
...
# Paths & config
# ──────────────────────────────────────────────────────────────────────────────
OUT_DIR    = Path(__file__).resolve().parent / "figs_clip_vision_sink_aware"
...
def plot_overlay(image_disp, cls_row, title="", with_values=False,
    ...
    norm_spatial = (spatial - sp_min) / (sp_max - sp_min)          # [0, 1]
    alpha_map    = (norm_spatial ** 0.5) * 0.65                     # max alpha = 0.65

    cmap_fn  = plt.get_cmap(COLORMAP)
    rgba     = cmap_fn(norm_spatial)                                # (H, W, 4)
    rgba[..., 3] = alpha_map                                        # set variable alpha
    ax.imshow(rgba)
...
def plot_k_sweep_overlays(image_disp, scores_by_k, k_values, title_prefix="",
    ...
    fig.suptitle(
        f"{title_prefix}  —  SA-RFEM, K-sweep over patch importance",
...
def rfem_k_sigma_filter_sink_aware(head_rollouts, k=0.5):
    """
    Sink-aware K-sigma filter for CLIP-Vision.

    μ_h, σ_h are computed from the CLS ROW ONLY (row 0), excluding column 0
    (the CLS-self entry). This prevents the dominant (0,0) sink value from
    inflating σ_h and killing all patch tokens at higher K values.
    Threshold is applied to the full T×T matrix (value-preserving).
    """
    H, T, _ = head_rollouts.shape
    device  = head_rollouts.device

    # Content columns: all patch token positions (cols 1..T-1), drop col 0 (CLS-self)
    content_cols = torch.arange(1, T, device=device)
    ...
    print(f"  Sink-aware filter: μ/σ from CLS row (row 0), excluding col 0 (CLS-self)")
    print(f"  {'Head':>5}  {'mu':>12}  {'sigma':>12}  {'threshold':>12}  {'kept':>16}")
    print(f"  {'-' * 66}")

    for h in range(H):
        R_h              = head_rollouts[h]
        cls_row_patches  = R_h[0, content_cols]   # 49 patch scores from CLS row
        mu_h             = cls_row_patches.mean()
        sigma_h          = cls_row_patches.std(unbiased=False)
...
def process_image(image_path: Path):
    ...
    for K in K_VALUES:
        ...
        plot_overlay(
            image_disp, agg[0],
            title=f"{img_id} Step 3 — SA-RFEM aggregated CLS overlay  (K={K})",
            save_name=f"05_rfem_step3_aggregate/K{K}/overlay",
        )
        plot_overlay(
            image_disp, agg[0], with_values=True,
            title=f"{img_id} Step 3 — SA-RFEM aggregated CLS overlay (values)  (K={K})",
